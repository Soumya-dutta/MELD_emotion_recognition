...
def test_model(PATH, test_imgs, test_aud, test_text, test_labels, test_lengths, test_speaker_mask, max_length):
    ...
    test_model = FusionModule(GRU_DIM*4, NUM_HIDDEN_LAYERS)
    model_path = PATH
    checkpoint = torch.load(model_path)
    #checkpoint = torch.load('ee_cross_best_model.tar')
    ...
    logger.info(f"Accuracy of fold-  {acc_fold}")
